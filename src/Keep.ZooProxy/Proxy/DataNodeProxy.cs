...
namespace Keep.ZooProxy
{
    internal abstract partial class DataNodeProxy<T> : NodeProxy, IDataNode<T>
    {
        ...
        public DataNodeProxy(ZooKeeperClient zkClient, string name, string path) : base(zkClient, name, path) { }
        ...
        {
            var dver = force ? -1 : _stat?.getVersion() ?? -1;
            await Connection.setDataAsync(Path, data, dver).ConfigureAwait(false);
        }
    }
}
